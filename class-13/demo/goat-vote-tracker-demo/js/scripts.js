...
function storeOurGoats(){
  ...
  var jsonStringGoats = JSON.stringify(allGoats);
  // creare a new property in our localstorage 
  localStorage.setItem('goats',jsonStringGoats);
}
console.log('inital');
console.table(allGoats);
parseLocalStorage();
...
function update(previousGoatsArr){
  for (let index = 0; index < allGoats.length; index++) {
    allGoats[index].numberOfClicks = previousGoatsArr[index].numberOfClicks;
    allGoats[index].numberOfTimesShown = previousGoatsArr[index].numberOfTimesShown;
